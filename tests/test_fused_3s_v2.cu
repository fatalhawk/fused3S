...
void cpu_block_sparse_attention(int num_nodes, int feat_dim, const float* Q, const float* K, const float* V, float* O, const blockSparseFormat& format) {
    ...
    for (int w = 0; w < num_row_windows; ++w) {
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            for (size_t i = 0; i < S.size(); ++i) { P[i] = std::exp(S[i] - max_val); exp_sum += P[i]; }
            for (size_t i = 0; i < P.size(); ++i) P[i] /= exp_sum;
            for (int f = 0; f < feat_dim; ++f) {
                float out_val = 0.0f;
                for (size_t i = 0; i < P.size(); ++i) {
                    int gc = col_indices[i];
                    if (gc < num_nodes) out_val += P[i] * V[gc * feat_dim + f];
                }
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== Fused3S v3 (Register-Resident O) ===" << std::endl;
    ...
    std::vector<float> h_O_cpu(feat_size, 0.0f), h_O_gpu(feat_size, 0.0f);
    ...
    std::cout << "Computing CPU ground truth..." << std::endl;
    ...
    int ct = 256, cb = (feat_size + ct - 1) / ct;
    float2half_kernel<<<cb, ct>>>(d_Q_f32, d_Q, feat_size);
    float2half_kernel<<<cb, ct>>>(d_K_f32, d_K, feat_size);
    float2half_kernel<<<cb, ct>>>(d_V_f32, d_V, feat_size);
    ...
    std::cout << "Launching kernel (dim3(32," << WARPS << "))..." << std::endl;
    int nrw = format.num_row_windows;
    dim3 block_dim(32, WARPS);
    dim3 grid_dim(nrw);
    ...
    f3s_1tb1rw_kernel<<<grid_dim, block_dim>>>(
        d_Q, d_K, d_V, d_O_out, d_window_offsets, d_block_col_indices, d_window_order, nrw, feat_dim);
    ...
    for (int i = 0; i < graph.num_nodes; ++i) {
        for (int j = 0; j < feat_dim; ++j) {
            float cv = h_O_cpu[i * feat_dim + j], gv = h_O_gpu[i * feat_dim + j];
            if (std::fabs(cv - gv) > tolerance) {
                passed = false;
                if (++mismatches <= 5)
                    std::cerr << "MISMATCH [" << i << "," << j << "]: cpu=" << cv << " gpu=" << gv << " d=" << std::fabs(cv-gv) << std::endl;
            }
        }
    }
    if (!passed) std::cerr << "Total mismatches: " << mismatches << "/" << graph.num_nodes * feat_dim << std::endl;
