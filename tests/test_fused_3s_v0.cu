...
// --- Exact Block-Sparse CPU Ground Truth ---
            ...
            for (int b = block_start; b < block_end; ++b) {
                ...
                for (int c = 0; c < 16; ++c) {
                    ...
                    if (global_col < num_nodes) {
                        ...
                        sum = -1e9f; // Mask out-of-bounds calculations
                    }
                    S.push_back(sum);
...
int main(int argc, char** argv) {
    ...
    // Padding nodes to the nearest multiple of 16 for memory allocation
    ...
    int threads = 256;
    int blocks = (feat_size + threads - 1) / threads;
    float2half_kernel<<<blocks, threads>>>(d_Q_f32, d_Q, feat_size);
    float2half_kernel<<<blocks, threads>>>(d_K_f32, d_K, feat_size);
    float2half_kernel<<<blocks, threads>>>(d_V_f32, d_V, feat_size);
    ...
    // Execute Final Fused3S Kernel
    std::cout << "Launching Full Fused Tensor Core Pipeline..." << std::endl;
    ...
    f3s_1tb1rw_kernel<<<num_row_windows, threads_per_block>>>(
    ...
    bool passed = true;
    float tolerance = 0.05f; // Threshold accounts for FP16 chained multiplication drift
    ...
    for (int i = 0; i < graph.num_nodes; ++i) {
        for (int j = 0; j < feat_dim; ++j) {
            ...
            if (std::fabs(cpu_val - gpu_val) > tolerance) {
                passed = false;
                std::cerr << "Mismatch at node " << i << ", feat " << j << ": CPU=" << cpu_val << ", GPU=" << gpu_val << std::endl;
                goto end_verify;
            }
        }
    }
    ...
    if (passed) {
        std::cout << "SUCCESS: Full Fused 3S Kernel matches exact CPU baseline!" << std::endl;
    } else {
